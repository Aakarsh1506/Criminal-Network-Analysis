...
CASES_SQL = "\n  SELECT COALESCE(source.properties->>'source_identifier', c.case_id) AS case_id, c.case_month, c.case_status, l.city, l.state, ct.crime_name\n  FROM cases c\n  LEFT JOIN extracted_entities source ON source.kind='Case' AND source.canonical_id=c.case_id\n  LEFT JOIN crime_types ct ON ct.crime_id = c.crime_id\n  LEFT JOIN locations l ON l.location_id = c.location_id\n  WHERE c.case_id IN (SELECT case_id FROM case_people WHERE person_id = %s)\n  ORDER BY c.case_month DESC\n"
