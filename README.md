...
│   └── server.py        # FastAPI entry point (`python -m backend1.server`)
│
├── frontend/
...
- `backend1/`: FastAPI equivalent of the above, same API routes and database schemas. See [`backend1/README.md`](backend1/README.md) for its own setup/run/verify instructions.
...
1. Create and activate a virtual environment for it, then install dependencies:
   ...
   source backend1/.venv/bin/activate   # Windows: backend1\.venv\Scripts\activate
   python -m pip install -r backend1/requirements.txt
   ```
2. Create `backend1/.env` using `backend1/.env.example` if you do not already have it, then configure it — this is a **separate file** from `express-backend/.env`, even if you're pointing both at the same databases.
3. With the virtual environment still active, start the API:
   ```bash
   npm run server:fastapi   # starts the FastAPI API (python3 -m backend1.server)
   npm run dev               # starts the Vite dev server, same as above
   ```

Open **http://localhost:3000** — same URL either way. See [`backend1/README.md`](backend1/README.md) for the full setup/run/verify reference, including the `add-officer` CLI equivalent (`npm run add-officer:fastapi`).
...
### Admin account
A single hardcoded account, checked against `ADMIN_USERNAME` / `ADMIN_PASSWORD` in your backend's `.env` (`express-backend/.env` or `backend1/.env`, depending which backend you're running) — **not** a row in the `officers` table. Logging in with these credentials on the normal login page redirects to `/admin` instead of `/dashboard`.
...
| Uploaded documents (Upload page) | `officer_documents` | Files stored on disk in `express-backend/uploads/` (or `backend1/uploads/` for FastAPI), metadata + ownership in Postgres |
...
2. Add `GROQ_API_KEY=your_key` to your backend's `.env` (`express-backend/.env` or `backend1/.env`). Keep existing database settings.
...
The FastAPI backend (`backend1/.env.example`) uses the same variable names — copy whichever backend's example file you're running to a real `.env` in that same folder. **Each backend reads its own `.env`; they don't share one.**
...
**Deploying with a managed Postgres provider?** Most give you a single `DATABASE_URL` connection string requiring SSL rather than discrete `PGHOST`/`PGUSER`/etc. — `express-backend/db.js` (or `backend1/db.py`) can be extended to prefer `DATABASE_URL` (with SSL enabled, e.g. `ssl: { rejectUnauthorized: false }` in Node) when it's present, falling back to the discrete vars for local dev.

**Deploying Neo4j by raw IP instead of a domain?** Public CAs won't issue certificates for bare IPs, so you'll be stuck with a self-signed certificate. Node's TLS stack may tolerate that as-is, but Python's `ssl` module won't — `backend1`'s driver needs its URI scheme rewritten from `neo4j+s://`/`bolt+s://` to `neo4j+ssc://`/`bolt+ssc://` to skip certificate verification. Prefer a real domain + CA-signed cert (or Neo4j Aura, which provides one automatically) wherever possible instead.
...
For the FastAPI backend, see [`backend1/README.md`](backend1/README.md#verify) for its `pytest`/`ruff` equivalents.
